...
// Referencia CPU serial (fuente de verdad del Lab 1)
...
// Verifica si existe hardware GPU CUDA disponible
...
TEST(CudaBufferTest, RoundTripH2DD2H) {
    if (!isCudaDeviceAvailable()) {
        GTEST_SKIP()
            << "No CUDA GPU detected. Skipping test.";
    }
...
TEST(CudaBufferTest, MoveTransfiereLaPropiedad) {
    ...
    // a quedó vacío: no habrá double-free.
    EXPECT_EQ(
        a.data(),
        nullptr
    );
...
//
// Con G=m2=d=1, eps=0.1:
// a1x = 1/(1.01)^1.5 ≈ 0.98519.
//
// El CPU serial del Lab 1 se considera la fuente de verdad.
...
TEST(GpuAccelerationsTest, CasoAnaliticoDosCuerpos) {
    ...
    for (int variant : {0, 1}) {
        ...
        // Acción-reacción con masas iguales:
        // a2x = -a1x.
        EXPECT_TRUE(
            NearTol(
                gpu_sys
...
// ============================================================
// Equivalencia CPU serial vs GPU
// ============================================================
...
// la variante shared debe producir el mismo resultado físico
// que la básica dentro de la tolerancia acordada.
...
// El resultado no debe depender de blockDim.x.
//
// Con N=100, que no es múltiplo de los tamaños de bloque,
// se ejercita la protección de bordes de la variante shared.
...
TEST(GpuAccelerationsTest, ParametrosInvalidosLanzan) {
    ...
    EXPECT_THROW(
        sys.computeAccelerationsGpu(99),
        std::invalid_argument
    );

    EXPECT_THROW(
        sys.computeAccelerationsGpu(0, 0),
        std::invalid_argument
    );

    EXPECT_THROW(
        sys.computeAccelerationsGpu(0, 2048),
        std::invalid_argument
    );
}
...
TEST(GpuIntegrationTest, StepEulerGpuCoincideConSerial) {
    ...
    // La energía se calculó en host en ambos casos:
...
// ============================================================
// Energía GPU: reducción shared y atomicAdd
// ============================================================
...
TEST(GpuEnergyTest, ReduccionYAtomicCoincidenConCpu) {
     ...
     * Asignamos velocidades deterministas para probar
     * realmente la energía cinética.
     */
    auto& bodies =
        simulator
            .getSystem()
    ...
    // Referencia CPU serial del Lab 1.
    ...
    // Método GPU por defecto:
    // reducción utilizando shared memory.
    ...
    // Método GPU 1:
    // acumulación mediante atomicAdd.
    ...
    // Ambas variantes GPU también deben coincidir.
    EXPECT_TRUE(
        NearTol(
            atomic_energy.first,
            reduction_energy.first
...
// Este test verifica la validación de argumentos antes
// de intentar ejecutar trabajo en la GPU.
// Por ello también puede ejecutarse en un runner sin GPU.
...
TEST(GpuEnergyTest, UnCuerpoTienePotencialCero) {
    ...
    // Energía calculada en GPU por reducción shared.
